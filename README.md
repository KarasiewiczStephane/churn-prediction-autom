...
> Automated machine learning pipeline for predicting customer churn using the Telco Customer Churn dataset. Combines H2O AutoML, Optuna-tuned LightGBM, and Logistic Regression with business impact analysis.

## Features

- **Data Pipeline**: Automated download from Kaggle, validation, preprocessing with stratified splitting
...
- **AutoML Training**: H2O AutoML with stacked ensembles and automatic model selection
- **Manual Optimization**: Optuna-tuned LightGBM and Logistic Regression with cross-validation
...
- **CI/CD**: GitHub Actions pipeline with linting, testing, coverage enforcement, and Docker build
...
│              │    │              │    │              │
│ - Download   │    │ - Correlation│    │ - H2O AutoML │
│ - Validate   │    │ - Mutual Info│    │ - LightGBM   │
│ - Preprocess │    │ - Boruta     │    │ - Log. Reg.  │
└──────────────┘    └──────────────┘    └──────┬───────┘
                                               │
┌──────────────┐    ┌──────────────┐           v
│   Business   │<───│    Model     │<──────────┘
...
python -m src.cli predict --input customers.csv --output predictions.csv
...
|---------------------|---------|----------|-----------|--------|---------------|
| H2O Stacked Ensemble| 0.847   | 0.612    | 0.654     | 0.575  | 300s          |
| Optuna LightGBM     | 0.841   | 0.598    | 0.642     | 0.560  | 180s          |
| Logistic Regression  | 0.804   | 0.542    | 0.589     | 0.502  | 45s           |

*Results from the Telco Customer Churn dataset (7,043 customers). Metrics computed on a held-out 20% test set.*
